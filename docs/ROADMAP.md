...
> Base técnica e funcionalidades core já implementadas.

### 0.1 Autenticação de usuários 

```
POST /api/auth/register  → cria conta (nome, celular, senha com bcrypt)
POST /api/auth/login     → retorna JWT com expiração de 7 dias
```

- Senhas com hash via `bcrypt`
- Token JWT assinado e verificado
- Middleware/Plug `authenticate` protege todas as rotas privadas (extrai `userId` do token)
...
| `direction` | `enum` | `OUT` (saída) ou `IN` (entrada) |
...
| Valor | Descrição |
|-------|-----------|
| `MERCADO` | Compras em supermercado e feira |
| `RESTAURANTES` | Refeições fora, delivery, lanche |
| `PRODUTOS_LIMPEZA` | Itens de limpeza e higiene doméstica |
| `SAUDE` | Consultas, plano de saúde, remédios |
...
| `EDUCACAO` | Cursos, livros, escola |
| `COMPRAS` | Roupas, eletrônicos, compras em geral |
| `DIVIDAS` | Parcelas, empréstimos, cartão de crédito |
...
- Tratamento centralizado de erros (`AppError`, `ValidationError`, `ResourceNotFoundError`)
...
| `inviteCode` | `varchar(8)` | Código único para convidar membros |
| `createdAt` | `timestamp` | — |
...
Adicionar coluna `householdId uuid` (nullable inicialmente para não quebrar dados existentes). O `userId` já existente passa a indicar *quem* fez o gasto; o `householdId` indica *de qual grupo* é.

### Endpoints novos

```
POST   /api/households                → cria um household (usuário vira OWNER)
POST   /api/households/join           → entra com inviteCode (vira MEMBER)
...
```
GET /api/expenses?memberId=<userId>
```

O parâmetro `memberId` é opcional. Sem ele, retorna todas as despesas do household.

### Fluxo de uso

```
1. Você cria o household → recebe um inviteCode (ex: "ABC12345")
2. Sua esposa cria a conta dela
3. Ela entra no household usando o inviteCode
4. Ambos passam a ver as despesas do grupo
5. Cada um lança as próprias despesas normalmente
6. Dá pra filtrar por pessoa: GET /expenses?memberId=<id-da-esposa>
```

### Impacto nas rotas existentes

Após a migração, `GET /expenses` deve retornar as despesas do **household** do usuário (não só as dele). Usuários sem household continuam vendo apenas as próprias despesas (comportamento atual preservado).
...
3. `ALTER TABLE expenses ADD COLUMN household_id uuid REFERENCES households(id)`
4. Índices: `(household_id)`, `(household_id, user_id)`, `(household_id, created_at DESC)`
...
[ ] Fase 7 — Households (gastos em família)
    ...
    [ ] Filtro memberId em GET /expenses
